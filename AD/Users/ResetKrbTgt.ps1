...
.AUTHOR
	...
	Feedback......................: Through contact details of blog: https://jorgequestforknowledge.wordpress.com/contact/
...
.KNOWN ISSUES/BUGS
	- (NOT APPLICABLE ANYMORE!) By default the script requires English language due to checking text output. There is a workaround though which
		is explained when the detects another language!
...
.NOTES
	To execute this script, the account running the script MUST have AT LEAST Domain Admin equivalent permissions in the targeted AD domain.
	If the account used is from another AD domain in the same AD forest, then the account running the script MUST have AT LEAST Enterprise Admin
	equivalent permissions in the targeted AD domain
	This is due to the reset of the password for the targeted KrbTgt account and forcing (single object) replication between DCs
#>
...
Function loadPoSHModules($PoSHModule) {
	$retValue = $null
	If(@(Get-Module | ?{$_.Name -eq $PoSHModule}).count -eq 0) {
		If(@(Get-Module -ListAvailable | ?{$_.Name -eq $PoSHModule} ).count -ne 0) {
...
Function createTempCanaryObject($targetedADdomainRWDC, $krbTgtSamAccountName, $execDateTimeCustom1) {
...
Function setPasswordOfADAccount($targetedADdomainRWDC, $krbTgtSamAccountName) {
	...
	$metadataObjectBeforeAttribPwdLastSet = $metadataObjectBefore | ?{$_.AttributeName -eq "pwdLastSet"}
	...
	$metadataObjectAfterAttribPwdLastSet = $metadataObjectAfter | ?{$_.AttributeName -eq "pwdLastSet"}
...
Function replicateSingleADObject($sourceDCNTDSSettingsObjectDN, $targetDCFQDN, $objectDN, $contentScope) {
...
Function deleteTempCanaryObject($targetedADdomainRWDC, $targetObjectToCheckDN) {
...
Function checkADReplicationConvergence($targetedADdomainFQDN, $targetedADdomainSourceRWDCFQDN, $targetObjectToCheckDN, $listOfDCsToCheckObjectOnStart, $listOfDCsToCheckObjectOnEnd, $modeOfOperationNr) {
		...
		ForEach ($dcToCheck in $listOfDCsToCheckObjectOnStart) {
			...
			If ($dcToCheckReachability) {
				...
				If ($dcToCheckHostName -ne $targetedADdomainSourceRWDCFQDN) {
					...
					replicateSingleADObject $sourceDCNTDSSettingsObjectDN $dcToCheckHostName $targetObjectToCheckDN $contentScope
				}
			...
			If ($dcToCheckReachability) {
				...
				If ($targetObjectToCheck -Or $objectOnTargetDCPwdLastSet -eq $objectOnSourceOrgRWDCPwdLastSet) {
					...
					If (!($listOfDCsToCheckObjectOnEnd | ?{$_."Host Name" -eq $dcToCheckHostName})) {
						# Define The Columns For This DC To Be Filled In
						$listOfDCsToCheckObjectOnEndObj = "" | Select "Host Name",PDC,"Site Name","DS Type","IP Address",Reachable,"Source RWDC FQDN",Time
				...
				# When The DC To Check Is Not Reachable
				Logging "     * Unable to connect to DC and check for the Temp Canary Object..." "ERROR"
				Logging "" "WARNING"
				# If The DC To Check Does Not Yet Exist On The Ending List With All DCs That Were Checked, Then Add It To The Ending List
				If (!($listOfDCsToCheckObjectOnEnd | ?{$_."Host Name" -eq $dcToCheckHostName})) {
					# Define The Columns For This DC To Be Filled In
	...
	If ($modeOfOperationNr -eq 2) {
		...
		If ($targetObjectToCheck) {
			...
			deleteTempCanaryObject $targetedADdomainSourceRWDCFQDN $targetObjectToCheckDN
		}
	}

	# Sort The Ending List With All DCs That Were Checked
	$listOfDCsToCheckObjectOnEnd = $listOfDCsToCheckObjectOnEnd | Sort-Object -Property @{Expression = "Time"; Descending = $False} | FT -Autosize
	Logging ""
...
Function createTestKrbTgtADAccount($targetedADdomainRWDC, $krbTgtSamAccountName, $krbTgtUse) {
	# Determine The DN Of The Default NC Of The Targeted Domain
	$targetedADdomainDefaultNC = $null
	$targetedADdomainDefaultNC = (Get-ADRootDSE -Server $targetedADdomainRWDC).defaultNamingContext
	# Determine The DN Of The Users Container Of The Targeted Domain
	...
	If ($krbTgtUse -eq "RWDC") {
		Logging "  --> Made Member Of RODC PRP Group.........: 'Denied RODC Password Replication Group'"
	}
	# If The Test/Bogus KrbTgt Account Is Used By RODCs
	If ($krbTgtUse -eq "RODC") {
		Logging "  --> Made Member Of RODC PRP Group.........: 'Allowed RODC Password Replication Group'"
	}
	Logging ""
	...
	If ($testKrbTgtObject) {
		...
		# If The Test/Bogus KrbTgt Account Is Used By RWDCs
		If ($krbTgtUse -eq "RWDC") {
			# Check If The Test/Bogus KrbTgt Account Is Already A Member Of The Specified AD Group On The Targeted Originating RWDC
			$membershipDeniedPRPGroup = $null
			$membershipDeniedPRPGroup = Get-ADGroupMember -Identity 'Denied RODC Password Replication Group' -Server $targetedADdomainRWDC | ?{$_.distinguishedName -eq $testKrbTgtObjectDN}
			If ($membershipDeniedPRPGroup) {
				# If The Test/Bogus KrbTgt Account Is Already A Member Of The Specified AD Group On The Targeted Originating RWDC
				
				Logging "  --> Test KrbTgt Account [$testKrbTgtObjectDN] ALREADY MEMBER OF [Denied RODC Password Replication Group]!..." "REMARK"
				Logging "" "REMARK"
			} Else {
				# If The Test/Bogus KrbTgt Account Is Not Yet A Member Of The Specified AD Group On The Targeted Originating RWDC, Then Add It As A Member
				
				Add-ADGroupMember -Identity 'Denied RODC Password Replication Group' -Members $testKrbTgtObjectDN -Server $targetedADdomainRWDC
				Logging "  --> Test KrbTgt Account [$testKrbTgtObjectDN] ADDED AS MEMBER OF [Denied RODC Password Replication Group]!..." "REMARK"
				Logging "" "REMARK"
			}
		}
		# If The Test/Bogus KrbTgt Account Is Used By RODCs
		If ($krbTgtUse -eq "RODC") {
			...
			$membershipAllowedPRPGroup = Get-ADGroupMember -Identity 'Allowed RODC Password Replication Group' -Server $targetedADdomainRWDC | ?{$_.distinguishedName -eq $testKrbTgtObjectDN}
			If ($membershipAllowedPRPGroup) {
				# If The Test/Bogus KrbTgt Account Is Already A Member Of The Specified AD Group On The Targeted Originating RWDC
				
				Logging "  --> Test KrbTgt Account [$testKrbTgtObjectDN] ALREADY MEMBER OF [Allowed RODC Password Replication Group]!..." "REMARK"
				Logging "" "REMARK"
			} Else {
				# If The Test/Bogus KrbTgt Account Is Not Yet A Member Of The Specified AD Group On The Targeted Originating RWDC, Then Add It As A Member
				
				Add-ADGroupMember -Identity 'Allowed RODC Password Replication Group' -Members $testKrbTgtObjectDN -Server $targetedADdomainRWDC
				Logging "  --> Test KrbTgt Account [$testKrbTgtObjectDN] ADDED AS MEMBER OF [Allowed RODC Password Replication Group]!..." "REMARK"
				Logging "" "REMARK"
			}
		}
	}
...
#$listOfSupportedISOLanguageCodes | %{
#	Logging "  --> $_"
#}
...
Logging ""
# Retrieve The AD Domain Of The Computer Where The Script Is Executed
...
$listOfADDomainsInADForest | %{
	...
	$domainObj = $null
	Try {
		$domainObj = Get-ADDomain $domainFQDN
	} Catch {
		$domainObj = $null
	}
	# Define The Columns For This AD Domain To Be Filled In
	$tableOfADDomainsInADForestObj = "" | Select Name,DomainSID,IsRootDomain,DomainMode,IsCurrentDomain,IsAvailable
...
Logging ""
Logging "$($tableOfADDomainsInADForest | FT | Out-String)"
Logging "  --> Found [$nrOfDomainsInForest] AD Domain(s) In The AD Forest..." "REMARK"
Logging ""
...
If ($targetedADdomainFQDN -eq "" -Or $targetedADdomainFQDN -eq $null) {
...
Logging ""
Logging "Checking existence of the specified AD domain '$targetedADdomainFQDN' in the AD forest..."
If ($adDomainValidity) {
	# If The AD Domain Is Valid And Therefore Exists, Continue
	
	Logging "" "SUCCESS"
	Logging "The specified AD domain '$targetedADdomainFQDN' exists in the AD forest!" "SUCCESS"
	Logging "" "SUCCESS"
	Logging "Continuing Script..." "SUCCESS"
	Logging "" "SUCCESS"
} Else {
	...
	Logging "" "ERROR"
	Logging "The specified AD domain '$targetedADdomainFQDN' DOES NOT exist in the AD forest!" "ERROR"
	Logging "" "ERROR"
	Logging "Please re-run the script and provide the FQDN of an AD domain that does exist in the AD forest..." "ERROR"
	Logging "" "ERROR"
	Logging "Aborting Script..." "ERROR"
	Logging "" "ERROR"
...
Logging "------------------------------------------------------------------------------------------------------------------------------------------------------" "HEADER"
Logging "TESTING IF REQUIRED PERMISSIONS ARE AVAILABLE (DOMAIN/ENTERPRISE ADMIN CREDENTIALS)..." "HEADER"
Logging ""

# Validate The User Account Running This Script Is A Member Of The Domain Admins Group Of The Targeted AD Domain
$targetedDomainObjectSID = ($tableOfADDomainsInADForest | ?{$_.Name -eq $targetedADdomainFQDN}).DomainSID
$domainAdminRID = "512"
...
If (!$userIsDomainAdmin) {
	...
	$forestRootDomainObjectSID = ($tableOfADDomainsInADForest | ?{$_.IsRootDomain -eq "TRUE"}).DomainSID
	...
	If (!$userIsEnterpriseAdmin) {
		...
		Logging "The user account IS NOT running with Domain/Enterprise Administrator equivalent permissions in the AD Domain '$targetedADdomainFQDN'!..." "ERROR"
		Logging "The user account IS NOT a member of '$domainAdminRole' and NOT a member of '$enterpriseAdminRole'!..." "ERROR"
		Logging "" "ERROR"
		...
		Logging "The user account is running with Enterprise Administrator equivalent permissions in the AD Domain '$targetedADdomainFQDN'!..." "SUCCESS"
		Logging "The user account is a member of '$enterpriseAdminRole'!..." "SUCCESS"
		Logging "" "SUCCESS"
		Logging "Continuing Script..." "SUCCESS"
		Logging "" "SUCCESS"
	}
	...
	Logging "The user account is running with Domain Administrator equivalent permissions in the AD Domain '$targetedADdomainFQDN'!..." "SUCCESS"
	Logging "The user account is a member of '$domainAdminRole'!..." "SUCCESS"
	Logging "" "SUCCESS"
	Logging "Continuing Script..." "SUCCESS"
	Logging "" "SUCCESS"
}
...
$thisADDomain = $null
Try {
	$thisADDomain = Get-ADDomain $targetedADdomainFQDN
} Catch {
	$thisADDomain = $null
}
If ($thisADDomain) {
	...
	$targetedADdomainRWDCWithPDCFSMONTDSSettingsObjectDN = (Get-ADDomainController $targetedADdomainRWDCWithPDCFSMOFQDN -Server $targetedADdomainRWDCWithPDCFSMOFQDN).NTDSSettingsObjectDN
	...
	$targetedADdomainDomainFunctionalModeLevel = (Get-ADObject -LDAPFilter "(&(objectClass=crossRef)(nCName=$('DC=' + $targetedADdomainFQDN.replace('.',',DC='))))" -SearchBase $partitionsContainerDN -Properties "msDS-Behavior-Version" -Server $targetedADdomainRWDCWithPDCFSMOFQDN)."msDS-Behavior-Version"
	# Determine The Max Tgt Lifetime In Hours And The Max Clock Skew In Minutes
	Try {
		$gpoObjXML = $null
		[xml]$gpoObjXML = Get-GPOReport -Domain $targetedADdomainFQDN -Guid '{31B2F340-016D-11D2-945F-00C04FB984F9}' -ReportType Xml
		$targetedADdomainMaxTgtLifetimeHrs = $null
		$targetedADdomainMaxTgtLifetimeHrs = (($gpoObjXML.gpo.Computer.ExtensionData | ?{$_.name -eq 'Security'}).Extension.ChildNodes | ?{$_.Name -eq 'MaxTicketAge'}).SettingNumber
		$targetedADdomainMaxClockSkewMins = $null
		$targetedADdomainMaxClockSkewMins = (($gpoObjXML.gpo.Computer.ExtensionData | ?{$_.name -eq 'Security'}).Extension.ChildNodes | ?{$_.Name -eq 'MaxClockSkew'}).SettingNumber
...
If ($listOfRWDCsInADDomain) {
	$listOfRWDCsInADDomain | %{
		# Get The FQDN Of The RWDC
		$rwdcFQDN = $null
		$rwdcFQDN = $_
		# Retrieve The Object Of The RWDC From AD
		$rwdcObj = $null
		$rwdcObj = Get-ADDomainController $rwdcFQDN -Server $targetedADdomainRWDCWithPDCFSMOFQDN
		# Define The Columns For The RWDCs In The AD Domain To Be Filled In
		$tableOfDCsInADDomainObj = "" | Select "Host Name",PDC,"Site Name","DS Type","Krb Tgt","Pwd Last Set","Org RWDC","Org Time","Ver","IP Address","OS Version",Reachable,"Source RWDC FQDN","Source RWDC DSA"
		...
		If ($rwdcKrbTgtObject) {
			...
			$metadataObjectAttribPwdLastSet = $metadataObject | ?{$_.AttributeName -eq "pwdLastSet"}
...
If ($listOfRODCsInADDomain) {
	$listOfRODCsInADDomain | %{
		# Get The FQDN Of The RODC
		$rodcFQDN = $null
		$rodcFQDN = $_
		# Get The FQDN Of The RODC
		$rodcObj = $null
		$rodcObj = Get-ADDomainController $rodcFQDN -Server $targetedADdomainRWDCWithPDCFSMOFQDN
		...
		If ($modeOfOperationNr -eq 1 -Or $modeOfOperationNr -eq 2 -Or $modeOfOperationNr -eq 4) {
			...
			$rodcKrbTgtSamAccountName = ((Get-ADObject $($rodcObj.ComputerObjectDN) -properties msDS-KrbTgtLink -Server $targetedADdomainRWDCWithPDCFSMOFQDN)."msDS-KrbTgtLink" | Get-ADObject -Server $targetedADdomainRWDCWithPDCFSMOFQDN).Name
		}
		If ($modeOfOperationNr -eq 3 -Or $modeOfOperationNr -eq 8 -Or $modeOfOperationNr -eq 9) {
			# Use The TEST/BOGUS KrbTgt Account Of The RODC
			
			$rodcKrbTgtSamAccountName = $(((Get-ADObject $($rodcObj.ComputerObjectDN) -properties msDS-KrbTgtLink -Server $targetedADdomainRWDCWithPDCFSMOFQDN)."msDS-KrbTgtLink" | Get-ADObject -Server $targetedADdomainRWDCWithPDCFSMOFQDN).Name) + "_TEST"
		}
		# Set The Corresponding Value Of The RODC In The Correct Column Of The Table
		...
		If ($rodcObj.OperatingSystem) {
			...
			If ($tableOfDCsInADDomainObj.Reachable -eq $True) {
				...
				$sourceRWDCsNTDSSettingsObjectDN | %{
					...
					# Connect To The Server Object DN
					$sourceRWDCServerObjectObj = ([ADSI]"LDAP://$targetedADdomainRWDCWithPDCFSMOFQDN/$sourceRWDCServerObjectDN")
...
Logging "" "REMARK"
Logging "$($tableOfDCsInADDomain | FT * -Autosize | Out-String)"
Logging "" "REMARK"
...
If ($modeOfOperationNr -eq 2 -Or $modeOfOperationNr -eq 3 -Or $modeOfOperationNr -eq 4) {
	...
	If ($targetKrbTgtAccountNr -eq 1) {
		...
		$krbTgtSamAccountName = ($tableOfDCsInADDomain | ?{$_.PDC -eq $True})."Krb Tgt"
		# Retrieve The Hosted Listed For The RWDC With The PDC FSMO
		$targetedADdomainSourceRWDCFQDN = $null
		$targetedADdomainSourceRWDCFQDN = ($tableOfDCsInADDomain | ?{$_.PDC -eq $True})."Host Name"
		...
		If ($targetedADdomainRWDCReachability) {
			...
			If ($modeOfOperationNr -eq 2) {
				...
				$targetObjectToCheckDN = $null
				$targetObjectToCheckDN = createTempCanaryObject $targetedADdomainSourceRWDCFQDN $krbTgtSamAccountName $execDateTimeCustom1
			...
			If ($modeOfOperationNr -eq 3 -Or $modeOfOperationNr -eq 4) {
				...
				If ($targetObjectToCheck) {
					...
					If ($okToReset) {
						...
						setPasswordOfADAccount $targetedADdomainSourceRWDCFQDN $krbTgtSamAccountName
					} Else {
		...
		If ($targetObjectToCheckDN) {
			...
			$listOfDCsToCheckObjectOnStart = ($tableOfDCsInADDomain | ?{$_."DS Type" -eq "Read/Write"})
			...
			checkADReplicationConvergence $targetedADdomainFQDN $targetedADdomainSourceRWDCFQDN $targetObjectToCheckDN $listOfDCsToCheckObjectOnStart $listOfDCsToCheckObjectOnEnd $modeOfOperationNr
		}
	}
	...
	If ($targetKrbTgtAccountNr -eq 2 -Or $targetKrbTgtAccountNr -eq 3) {
		...
		$collectionOfReachableRODCsToProcess = $tableOfDCsInADDomain | ?{$_."DS Type" -eq "Read-Only" -And $_."Source RWDC FQDN" -ne "Unknown" -And $_."Source RWDC FQDN" -ne "RODC Unreachable" -And $targetRODCFQDNList -contains $_."Host Name"}
		# Collection Of UnReachable RODCs Specified To Be Targeted
		$collectionOfUnReachableRODCsToProcess = $null
		$collectionOfUnReachableRODCsToProcess = $tableOfDCsInADDomain | ?{$_."DS Type" -eq "Read-Only" -And $_."Source RWDC FQDN" -eq "RODC Unreachable" -And $targetRODCFQDNList -contains $_."Host Name"}
		# Collection Of Unknown RODCs Specified To Be Targeted
		$collectionOfUnknownRODCsToProcess = $null
		$collectionOfUnknownRODCsToProcess = $tableOfDCsInADDomain | ?{$_."DS Type" -eq "Read-Only" -And $_."Source RWDC FQDN" -eq "Unknown" -And $targetRODCFQDNList -contains $_."Host Name"}
	}
	...
	If ($targetKrbTgtAccountNr -eq 4) {
		# Collection Of Reachable RODCs Specified To Be Targeted
		$collectionOfReachableRODCsToProcess = $null
		$collectionOfReachableRODCsToProcess = $tableOfDCsInADDomain | ?{$_."DS Type" -eq "Read-Only" -And $_."Source RWDC FQDN" -ne "Unknown" -And $_."Source RWDC FQDN" -ne "RODC Unreachable"}
		# Collection Of UnReachable RODCs Specified To Be Targeted
		$collectionOfUnReachableRODCsToProcess = $null
		$collectionOfUnReachableRODCsToProcess = $tableOfDCsInADDomain | ?{$_."DS Type" -eq "Read-Only" -And $_."Source RWDC FQDN" -eq "RODC Unreachable"}
		# Collection Of Unknown RODCs Specified To Be Targeted
		$collectionOfUnknownRODCsToProcess = $null
		$collectionOfUnknownRODCsToProcess = $tableOfDCsInADDomain | ?{$_."DS Type" -eq "Read-Only" -And $_."Source RWDC FQDN" -eq "Unknown"}
	}
	...
	If ($targetKrbTgtAccountNr -eq 2 -Or $targetKrbTgtAccountNr -eq 3 -Or $targetKrbTgtAccountNr -eq 4) {
		...
		If ($collectionOfReachableRODCsToProcess) {
			$collectionOfReachableRODCsToProcess | %{
				...
				$targetedADdomainSourceRWDCObj = Get-ADDomainController $targetedADdomainSourceRWDCFQDN -Server $targetedADdomainRWDCWithPDCFSMOFQDN
				...
				$targetedADdomainSourceRWDCReachability = ($tableOfDCsInADDomain | ?{$_."Host Name" -eq $targetedADdomainSourceRWDCFQDN}).Reachable
				...
				# Define The Columns For This DC To Be Filled In
				$listOfDCsToCheckObjectOnStartObj = "" | Select "Host Name",PDC,"Site Name","DS Type","IP Address",Reachable,"Source RWDC FQDN","Source RWDC DSA"
				# Set The Corresponding Value Of The DC In The Correct Column Of The Table
		...
		If ($collectionOfUnReachableRODCsToProcess) {
			$collectionOfUnReachableRODCsToProcess | %{
				# Retrieve The RODC Object In The List
				$rodcToProcess = $null
				$rodcToProcess = $_
		...
		If ($collectionOfUnknownRODCsToProcess) {
			...
			$collectionOfUnknownRODCsToProcess | %{
				$rodcToProcess = $null
				$rodcToProcess = $_
				Logging "$($rodcToProcess | FT * | Out-String)"
				Logging ""
			}
			Logging ""
		} Else {
...
If ($modeOfOperationNr -eq 8) {
	...
	createTestKrbTgtADAccount $targetedADdomainSourceRWDCFQDN $krbTgtSamAccountName "RWDC"

	# For All RODCs In The AD Domain That Do Not Have An Unknown RWDC Specfied
	$tableOfDCsInADDomain | ?{$_."DS Type" -eq "Read-Only" -And $_."Source RWDC FQDN" -ne "Unknown"} | %{
		# Retrieve The RODC Object In The List
		$rodcToProcess = $null
		$rodcToProcess = $_
		# Retrieve The sAMAccountName Of The KrbTgt Account In Use By The RODC
		...
		createTestKrbTgtADAccount $targetedADdomainSourceRWDCFQDN $krbTgtSamAccountName "RODC"
	}
}
...
If ($modeOfOperationNr -eq 9) {
	...
	deleteTestKrbTgtADAccount $targetedADdomainSourceRWDCFQDN $krbTgtSamAccountName
